/*
 * AssistantLinphoneAccountView.qml
...
ScrollView {
    ...
    Container {
        ...
        Label {
            text: qsTr("CONFIGURE LINPHONE ACCOUNT") + Retranslate.onLanguageChanged
            ...
            AssistantButton {
                ...
                onClicked: {
                    if (username.text.length == 0) {
                        showDialog(qsTr("Please fill in your username") + Retranslate.onLanguageChanged)
                    } else {
                        var success = assistantModel.configureLinphoneAccount(username.text, password.text, displayName.text)
                        if (success) {
                            navigationPane.pop();
                        }
                    }
